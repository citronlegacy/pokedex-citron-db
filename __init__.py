...
def get_by_generation(generation: int) -> List[str]:
    """
    Get all Pokémon from the specified generation.
    
    Args:
        generation: The generation number to search for (1-9)
        
    Returns:
        A list of Pokémon names from that generation
    """
    return [
        pokemon['name']
        for pokemon in _pokemon_data
        if pokemon.get('generation') == str(generation)
    ]
